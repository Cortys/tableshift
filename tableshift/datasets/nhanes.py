...
# each dataset.
NHANES_DATA_SOURCES = os.path.join(
    os.path.dirname(__file__), "nhanes_data_sources.json"
)
...
NHANES_CHOLESTEROL_DATA_SOURCES_TO_USE = {
    ...
    "Questionnaire": [
        "Blood Pressure & Cholesterol",  # All years
...
def get_nhanes_data_sources(task: str, years=None):
    ...
    years = [int(x) for x in years] if years is not None else []
    if task == "cholesterol":
...
NHANES_CHOLESTEROL_FEATURES = FeatureList(
    features=[
        Feature(
            "LBDLDL",
            float,
        ...
        Feature(
            "BPQ020",
            cat_dtype,
            """{Have you/Has SP} ever been told by a 
    ...
    diabetes?""",
            ...
            value_mapping={**DEFAULT_NHANES_CODING, "3.0": "Borderline"},
        ),
        ####### Risk Factor: Chronic kidney disease
        Feature(
            "KIQ025",
            cat_dtype,
            """In the past 12 months, {have you/has SP} 
...
NHANES_LEAD_FEATURES = FeatureList(
    features=[
        ...
        Feature(
            "LBXBPB", float, "Blood lead (ug/dL)", is_target=True, na_values=(".",)
        ),
    ],
    documentation="https://wwwn.cdc.gov/Nchs/Nhanes/",
)

NHANES_SHARED_FEATURES = FeatureList(
    features=[
        # Derived feature for survey year
        Feature("nhanes_year", int, "Derived feature for year.", name_extended="year"),
        Feature(
            "DMDBORN4",
            cat_dtype,
    ...
    1	Born in 50 US states or Washington, DC 2 Others""",
            ...
            value_mapping={
                "1.0": "born in 50 US states or Washington, DC",
    ...
    has} received?""",
        ...
        Feature(
            "RIDAGEYR",
            float,
            """Age in years of the participant at the time 
    of screening. Individuals 80 and over are topcoded at 80 years of age.""",
            name_extended="age in years",
        ),
        Feature(
            "RIAGENDR", cat_dtype, "Gender of the participant.", name_extended="gender"
        ),
        Feature(
            "DMDMARTL", cat_dtype, "Marital status", name_extended="marital status"
        ),
        Feature(
            "RIDRETH_merged",
            int,
            """Derived feature. This feature uses 
...
def preprocess_nhanes_cholesterol(df: pd.DataFrame, threshold=160.0):
